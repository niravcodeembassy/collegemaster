...
$(document).ready(function () {
  ...
  $(document).on(
    ...
    function (e) {
      ...
      html = $.map(list, function (last_item, indexs) {
        ...
        var diplytext = last_item
          .map(function (val, index) {
            return (
              '<span class="badge text-capitalize mb-1">' + val.text + "</span>"
            );
          })
          .join("&nbsp;");
  ...
  $(document).on(
    "change",
    '.variants_body input[name="is_variants[]"]',
    function (event) {
  ...
  $("#variation_form").validate({
    ...
    // ignore: [''],
    ignore:
      '.select2-search__field,:hidden:not("textarea,.files,select,variants_name")',
    rules: {
      "variant_price[]": {
        required: true,
      },
  ...
  function getselectTo(param) {
    ...
    $(".optionnameselect2").select2({
      ...
      ajax: {
        ...
        dataType: "json",
        processResults: function (data) {
          $selected = $(".optionnameselect2").find(":selected").text();
          var filtered = data.data.filter(function (value, index, arr) {
