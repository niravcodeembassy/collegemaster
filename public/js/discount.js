$(document).ready(function () {
    ...
    $('.generate-code').on('click', function () {
        var couponCode =  Math.random().toString(36).substring(3, 6) + Math.random().toString(36).substring(10, 12);
    ...
    $('.applies-to').on('change' , function(e){
        ...
        targetDiv.children().addClass('d-none').find('select').val(null).trigger('change');

        if((typeof target) !== 'undefined'){
            $(target).removeClass('d-none').find('select').val(null).trigger('change');
        }
