...
class Helper
{
  ...
  public static function productRouteParameter($product)
  {
    $arr = [];
    $arr['cat_slug'] = $product->category->slug;
    if ($product->sub_category_id !== null && isset($product->subcategory)) {
      $arr['product_subcategory_slug'] = $product->subcategory->slug;
      $arr['slug'] = $product->slug ?? 'all';
    } else {
      $arr['product_subcategory_slug'] = $product->slug ?? 'all';
      $arr['slug'] = null;
    }
    return $arr;
  }
