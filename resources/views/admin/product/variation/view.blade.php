...
@endcomponent
@php
    $action = isset($variation) ?
            route('admin.variation.variation_update_form' , [ 'product_id' => $product->id , 'id' => $variation->id ]) :
            route('admin.variation.variation_add_save' , [ 'product_id' => $product->id ]);
...
<form
    action="{{ $action }}"
    enctype="multipart/form-data" method="POST" name="variation_form"
    id="variation_form" autocomplete="off">
    @csrf
                            ...
                            @if($product->images->count() > 0)
                                @foreach ($product->images as $imgitem)
                                    <div class=" mb-3" style="width: 75px" id="select-image-{{$imgitem->id}}">
                                        ...
                                        <img
                                          src="{{ $imgitem->variant_image }}"
                                          @click="imageClick({{$item->id}},{{$imgitem->id}})"
                                          data-varint="{{ $item->id }}" data-id="{{$imgitem->id}}"
                                          class="w-100 rounded shadow card-img-top"
                                          :class="{ 'img-active': hasSelected({{ $item->id }},{{$imgitem->id}}) }"
                                          style="width: 56px;height: 56px;object-fit: contain;background: #e2e8f0;cursor: pointer;" srcset="">
                                    </div>
                                @endforeach
                            @else
                    ...
                    <div class="col-md-12 px-sm-3 px-2">
                        ...
                        @foreach($productvariantcombination as $key => $item)
                                ...
                                <div class="repeter-list">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="variants_id_{{ $loop->iteration }}"
                                            data-rule-required="true"
                                            data-target="#option_id_{{$option->id}}" data-url="{{ route('admin.get.optionvalue') }}"
                                            name="variants_id[{{ $loop->index }}]" data-msg-required="Variation is required." style="width: 100%;"
                                ...
                                <div class="repeter-list">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="variants_name_{{ $loop->iteration }}" data-rule-required="true"
                                            data-target="#option_id_{{$item->option->id}}" data-url="{{ route('admin.get.optionvalue') }}"
                                            name="variants_id[{{ $loop->index }}]" data-msg-required="Variation is required." style="width: 100%;"
                                            class="form-control text-lowercase variants_tags">
                                        </select>
                                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col">
                                ...
                                <input id="offer_price" name="offer_price"
                                    ...
                                    data-rule-required="false" data-rule-number="true"
                        ...
                        <div class="form-group">
                            ...
                            <input id="my-input" class="form-control" data-rule-required="true"
                                ...
                                data-msg-required="Quantity is required."
                                type="text" id="quantity" name="inventory_quantity" >
                        </div>
                            ...
                            <div class="card-body p-0 border show-img">
                                ...
                                <div class="form-group">
                                    <div class="text-center">
                                        @if(isset($variation))
                                            <img src="{{ $variation->image->variant_image ?
                                                            $variation->image->variant_image :
                                                            asset('storage/default/default.png') }}"
                                                data-default="{{ asset('storage/default/default.png') }}" class="w-100 " style="height: 220px;object-fit: cover;"
                                                id="preview">
                                        @else
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="justify-content-center d-flex mb-2">
                                        <button type="button" class="btn shadow btn-info "
                                        data-toggle="modal"
