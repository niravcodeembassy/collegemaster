                ...
                <div class="card-body">
                    ...
                    @if ($customer->orders->count() > 0)
                        <div class="profiletimeline mt-0">
                            ...
                            <div class="sl-item">
                                <div class="sl-right">
                                    <a href="{{ route('admin.order.show' ,$item->id) }}" class="link text-primary "> Order No. {{ $item->order_number ?? '---' }}
                                            <span class="sl-date   text-behance">{{ $item->created_at->format('d-m-Y') ?? '---' }} </span>
                                            @isset($item->order_status)
                                                <small class="text-muted  d-block f-12 pt-2">Order Status</small>
                                                <h6 class="mb-0 text-muted">{{ ucfirst(str_replace('_' ,' ' , $item->order_status)) }}</h6>
                                            @endisset
