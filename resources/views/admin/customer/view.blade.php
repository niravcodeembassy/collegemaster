                ...
                @foreach ($customer->wishlist as $item)
                  <div class="sl-item mt-4">
                    <div class="sl-right">
                      ...
                      @php
                        $variant = App\Model\Productvariant::with('product.image')
                            ->where('id', $item->variant_id)
                            ->first();
                        $attributes = json_decode($variant->variants, true);
                        $image_id = $variant->product->image->id;
                        $image_url = null;
                        if (isset($image_id)) {
                      ...
                      <div class="feeds-widget p-0">
                        <div class="feed-item border-0 p-0">
                          <a href="{{ route('admin.product.edit', $variant->product_id) }}" class="d-flex align-items-start">
                            <div class="feeds-left d-inline-block " style="width:20%;">
                              <div class="text-center">
                                <img src="{{ $image_url }}" style="width:80%" alt="">
                              </div>

                            </div>
                            <div class="feeds-body">
                              <h6 class="font-weight-normal f-18 text-primary">{{ $variant->product->name }}</h6>
