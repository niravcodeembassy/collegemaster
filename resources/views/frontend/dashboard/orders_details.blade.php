                ...
                <div class="row">
                    <div class="col-md-8 mb-8">
                        <h5 class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div>
                                <a href="{{ route('order.inv', $order->id) }}" class="text-muted text-strong mr-5"
                                    style="cursor: pointer;"> <i class="fa fa-print"></i> View Invoice </a>
                                <span
                                    class="badge  badge-secondary badge-pill">{{ $order->created_at->format('m-d-Y H:i:s') }}
                                    (IST)</span>
                            </div>
                        </h5>
                        <ul class="list-group mb-3  b-0">
                            ...
                            @foreach ($order->items as $item)
                                ...
                                <li class="list-group-item d-flex justify-content-between lh-condensed order-table">
                                    <img src="{{ $image_url }}" title="{{ $item->name }}" alt="{{ $item->name }}"
                                        style="width: 160px; height:160px;" class="mr-4">
                                    <div class="product-details">
                                        <h6 class="my-0 f-18 p-name text-muted" title="{{ $item->name }}">
                                            {{ Str::words($item->name, 10, '...') }}</h6>
                                        ...
                                        @if ((isset($raw_data['notes']) && $raw_data['notes'] != '') ||
                                            (isset($raw_data['gift_message']) && $raw_data['gift_message'] != ''))
                                            <hr>
                                            @if (isset($raw_data['notes']) && $raw_data['notes'] != '')
                                                <small class="text-muted f-12" style="margin-top: 5px;"><span
                                                        class="text-uppercase">Notes</span> :
                                                    {{ $raw_data['notes'] ?? '' }}</small><br>
                                            @endif
                      ...
                      @endif --}}
                                            ...
                                            @if (isset($raw_data['gift_message']) && $raw_data['gift_message'] != '')
                                                <small class="text-muted f-12" style="margin-top: 3px;"><span
                                                        class="text-uppercase">Gift Message</span> :
                                                    {{ $raw_data['gift_message'] ?? '' }}</small>
                                                <br>
                                            @endif
                                            @if (isset($raw_data['optional_note']) && $raw_data['optional_note'] != '')
                                                <small class="text-muted f-12" style="margin-top: 3px;"><span
                                                        class="text-uppercase">Optional Note </span>:
                                                    {{ $raw_data['optional_note'] ?? '' }}</small>
                                                <br>
                                            @endif
                                        @endif
                                    </div>
                                    @if (in_array($order->order_status, ['order_placed', 'pick_not_receive', 'work_in_progress', 'correction']) &&
                                        !$item->images->count() > 0)
                                        <div data-url="{{ route('cart.load.popup.ordered', ['item' => $item->id]) }}"
                                            class="text-center load-image-popup">
                                    ...
                                    @if ($item->images->count())
                                        <div class="d-flex flex-column align-items-center">
                                            ...
                                            <span class="text-success text-center"
                                                style="font-size: 11px;line-height:initial">
                    ...
                    <div class="col-md-4 ">
                        ...
                        <div class="card shadow-none border">
                            <div class="card-body">
                                ...
                                @endif
                                @if ($shipping->mobile)
                                    <span class="text-muted "> <strong>Mobile</strong> : {{ $shipping->mobile }}</span>
                                    <br>
                                @endif
                                ...
                                @endif
                                <span class="text-muted text-uppercase"> {{ $shipping->city ?? '' }} -
                                    {{ $shipping->postal_code ?? '' }} </span> <br>
                                <span class="text-muted text-uppercase">{{ $shipping->state ?? '' }} -
                                    {{ $shipping->country ?? '' }}</span> <br>
                            </div>
                        </div>
                            ...
                            <div class="card shadow-none border">
                                <div class="card-body">
                                    @if ($billing->billing_email)
                                        <span class="text-muted"> <strong>Email</strong> :
                                            {{ $billing->billing_email }}</span> <br>
                                    @endif
                                    @if ($billing->billing_mobile)
                                        <span class="text-muted"> <strong>Mobile</strong> :
                                            {{ $billing->billing_mobile }}</span> <br>
                                    @endif
                                    @if ($billing->billing_address_one)
                                        <span class="text-muted text-uppercase">{{ $billing->billing_address_one }}</span>
                                        <br>
                                    @endif
                                    @if ($billing->billing_address_two)
                                        <span class="text-muted text-uppercase"> {{ $billing->billing_address_two }}</span>
                                        <br>
                                    @endif
                        ...
                        <div class="card shadow-none border">
                            <div class="card-body">
                                ...
                                <span class="text-muted">
                                    {{ $order->payment_type == 'cash' ? 'COD' : $order->payment_type }}</span> <br>
                                <hr>
                                ...
                                @if ($order->order_status == 'delivered')
                                    ...
                                    <span class="text-muted">Delivery Date :
                                        {{ isset($order->deleverd_date) ? date('F j, Y', strtotime($order->deleverd_date)) : '' }}
                                    </span> <br>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == "success") {
                            ...
                            $('#msg_send_btn').html(
                                'Send <i class="ion-android-send ml-2"></i>');
