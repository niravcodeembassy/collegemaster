@if ($new_option_lable == 'size')
    ...
    <div class="form-group mb-25    ">
        <label for="variatoins_1" class="d-block shop-product__block__title">{{ ucfirst('Size') }}</label>
        <div class="d-block clearfix " style="width: 30%;">
            <select name="variatoins" class="form-control change-combination " id="variatoins_1" style="width: 250px;">
                @foreach ($size_item as $item)
                    ...
                    @endif
                    <option value="{{ $item['id'] }}" data-optionvalue="{{ $item['optionvalue'] }}"
                        data-option="{{ $item['option'] }}" @if ($item['optionvalue'] == $optionVal) {{ 'selected' }} @endif>
                        {{ $item['optionvalue'] }} @if ($item['optionvalue'] != $optionVal) - (${{ min($item['price']) }} - ${{ max($item['price']) }}) @endif
                    </option>
                @endforeach
            </select>
        </div>
    ...
    <div class="form-group mb-25    ">
        <label for="variatoins_2" class="d-block shop-product__block__title">{{ ucfirst('Printing options') }}</label>
        <div class="d-block clearfix " style="width: 30%;">
            <select name="variatoins" class="form-control change-combination " id="variatoins_2" style="width: 250px;">
                @foreach ($new_option as $key => $item)
                    @if ($loop->first)
                        <option value=""> Select </option>
                    @endif
                    <option value="{{ $item }}" data-optionvalue="{{ $item }}"
                        data-option="printing options">
                        {{ $item }} - ${{ $price[$key] }}
                    </option>
                @endforeach
            </select>
...
@else
    ...
    <div class="form-group mb-25    ">
        <label for="variatoins_1"
            class="d-block shop-product__block__title">{{ ucfirst('Printing options') }}</label>
        <div class="d-block clearfix " style="width: 30%;">
            <select name="variatoins" class="form-control change-combination " id="variatoins_1" style="width: 250px;">
