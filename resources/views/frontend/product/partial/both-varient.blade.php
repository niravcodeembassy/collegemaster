...
<div style="display: none">
    <span id="new_main_price">$ {{ $mrp_price }}</span>
    <span id="new_discounted_price">$ {{ $new_price }}</span>
    ...
    <div class="d-block clearfix " style="width: 30%;">
        <select name="variatoins" class="form-control change-combination " id="variatoins_2" style="width: 250px;">
            @foreach ($size_new_option as $key => $item)
                ...
                @endif
                <option value="{{ $item }}" data-optionvalue="{{ $item }}" @if ($selectBoxval['size'] == $item) {{ 'selected' }} @endif
                    data-option="size">
                    {{ $item }} @if ($selectBoxval['size'] != $item) -
                        ${{ $size_price[$key] }} @endif
                </option>
            @endforeach
        </select>
    </div>
        ...
        <select name="variatoins" class="form-control change-combination " id="variatoins_1" style="width: 250px;">
            @foreach ($printing_new_option as $key => $item)
                ...
                <option value="{{ $item }}" data-optionvalue="{{ $item }}" @if ($selectBoxval['printing options'] == $item) {{ 'selected' }} @endif data-option="printing options">
                    {{ $item }} @if ($selectBoxval['printing options'] != $item) - ${{ $printing_price[$key] }}
                    @endif
                </option>
            @endforeach
        </select>
