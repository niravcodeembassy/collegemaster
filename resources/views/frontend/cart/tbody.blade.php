...
@forelse($cartList as $cart)
    ...
    <tr>
        <td class="product-thumbnail pb-15 px-3" colspan="2">
            <a
                href="{{ route('product.details', ['slug' => $cart->product->slug, 'variant' => $cart->productvariant->id]) }}">
        ...
        <td class="product-name pb-15 px-3">
            <a class="mb-10" data-toggle="tooltip" data-placement="top" title="{{ $cart->product->name }}"
                href="{{ route('product.details', ['slug' => $cart->product->slug, 'variant' => $cart->productvariant->id]) }}">{{ Str::words($cart->product->name, 10, '...') }}</a>
            {!! $variant !!}
        </td>
        <td class="product-price pb-15 px-3"><span
                class="price">{{ $priceData->price ?? $priceData->offer_price }}</span></td>
        <td class="product-quantity pb-15 px-3 text-center">
            <div style="height: 60px;margin-top:15px;">
                <div class="pro-qty d-inline-block py-0 mx-0">
                    <input type="text" value="{{ $cart->qty }}" class="add-to-cart-update-view border-0" min="1"
                        name="view_update_{{ $cart->id }}" readonly data-cart="{{ json_encode($cartData) }}"
