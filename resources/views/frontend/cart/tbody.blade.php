...
@forelse($cartList as$key=> $cart)
    ...
    <tr>
        <td class="product-thumbnail pb-15 px-3" colspan="2">
            <a href="{{ route('product.view', $routeParameter) }}">
                @if ($cart->productvariant->productimage_id && optional($cart->image)->variant_image)
                    <img src="{{ $cart->image->variant_image }}" class="img-fluid " style="width: 100px" alt="variant"
                        title="variant">
                @else
                    <img src="{{ asset('storage/default/default.png') }}" style="width: 100px" class="img-fluid"
        ...
        <td class="product-name pb-15 px-3">
            <a class="mb-10" data-tippy="{{ $cart->product->name ?? '' }}" data-tippy-placement="top"
                data-tippy-inertia="true" data-tippy-animation="shift-away" data-tippy-delay="50"
                data-tippy-arrow="true"
                href="{{ route('product.view', $routeParameter) }}">{{ Str::words($cart->product->name, 10, '...') }}</a>
            {!! $variant !!}
        </td>
        <td class="product-price pb-15 px-3"><span
                class="price">{{ $priceData->price ?? $priceData->offer_price }}</span>
        </td>
        <td class="product-quantity pb-15 px-3 text-center">
            <div>
                <div class="pro-qty d-inline-block">
                    <input type="text" value="{{ $cart->qty }}" class="add-to-cart-update-view border-0"
                        min="1" name="view_update_{{ $cart->id }}" readonly
                        data-cart="{{ json_encode($cartData) }}" data-cartnotes="{{ $cart->notes }}"
                        data-url="{{ route('cart.view-update', ['update' => true]) }}"
                        data-msg-max="Product is out of stock" max="{{ $cart->productvariant->inventory_quantity }}">
                </div>
                <div class="errors"></div>
            </div>
        </td>
        <td data-url="{{ route('cart.load.popup', ['cart' => $cart->id]) }}"
            class="text-center load-image-popup  pb-15 px-3">
            <style>
            ...
            @else
                ...
                @endauth
                <div class="text-info d-sm-block d-none" style="margin-top:4px;font-size: 12px;line-height:initial">
                    Upload Pictures <br>
                    (Optional)
                </div>
                <div class="text-info d-sm-none upload_pic_car">
            ...
            <input type="hidden" class="attachment" name="attachment_{{ $cart->id }}"
                id="attachment_{{ $cart->id }}" value="{{ $imageCount }}"
                data-msg-max="Please upload {{ $uploadimage }} attachment"
        ...
        <td class="product-remove pb-15 px-3">
            <a href="javascript:void(0);" class="add-to-cart-remove-view" data-url="{{ route('cart.view-remove') }}"
                data-id="{{ $cart->id }}">
    ...
    @auth
        <tr style="border-bottom:none;" class="px-3 gift_message_area">
            <td colspan="7" class="px-3" style="border-top:none;">
                <div class="d-block">
                    <label class="switch mr-2">
                        <input type="hidden" value="{{ $cart->id }}" name="cart[{{ $key }}]cart_id[]"
                            class="cart_id">
            ...
            <span class="slider round"></span> --}}
                        ...
                        <input type="checkbox" class="enable_message enable_gift"
                            value="{{ $cart->order_has_message ?? '' }}"
                            name="cart[{{ $key }}]order_has_message[]" id="enable_{{ $cart->id }}"
          ...
          <span>Add Gift Message For Free</span> --}}
                    <div class="gift_message">
                        <textarea name="cart[{{ $key }}]gift_message[]" rows="3"
                            class="col-lg-6 col-12 order_gift_message form-control"
                            placeholder="Enter Your message - make sure to include to/from names">{{ $cart->gift_message ?? '' }}</textarea>
                        <label class="col-lg-6 col-12">Use the space above to enter your gift message. Please make sure to
                            include to/from names.</label>
                    </div>
                </div>
                <textarea name="cart[{{ $key }}]optional_note[]" rows="3"
                    class="mt-4 col-lg-6 col-12 order_optional_note form-control"
                    placeholder="Add a note to CollagemasterCo (optiontal)">{{ $cart->optional_note ?? '' }}</textarea>
            </td>
        </tr>
    @endauth
@empty
