...
$product
=App\Model\Product::where('slug',$cart->product->slug)->select('id','slug','category_id','sub_category_id')->first();
...
<tr>
  ...
  <td class="product-name pb-15 px-3">
    <a class="mb-10" data-toggle="tooltip" data-placement="top" title="{{ $cart->product->name }}"
      href="{{ route('product.details', $routeParameter) }}">{{
      Str::words($cart->product->name, 10, '...') }}</a>
    {!! $variant !!}
  </td>
    ...
    <div style="height: 60px;margin-top:15px;">
      <div class="pro-qty d-inline-block py-0 mx-0">
        <input type="text" value="{{ $cart->qty }}" class="add-to-cart-update-view border-0" min="1"
          ...
          data-url="{{ route('cart.view-update', ['update' => true]) }}" data-msg-max="Product is out of stock"
          max="{{ $cart->productvariant->inventory_quantity }}">
      </div>
  ...
  <td data-url="{{ route('cart.load.popup', ['cart' => $cart->id]) }}" class="text-center load-image-popup  pb-15 px-3">
    ...
    <input type="hidden" class="attachment" name="attachment_{{ $cart->id }}" id="attachment_{{ $cart->id }}"
      value="{{ $imageCount }}" data-msg-max="Please upload {{ $uploadimage }} attachment"
